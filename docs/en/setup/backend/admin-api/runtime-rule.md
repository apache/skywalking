...
The unified security notice for `admin-server` and every endpoint hosted on it
(including this one) lives in [Admin API → Introduction](readme.md#-security-notice).
Read it before enabling this module — it covers gateway protection, private-interface
binding, the public-internet posture, and access auditing.
...
The runtime-rule module uses **two ports** owned by `admin-server`:
- `17128` (HTTP REST) — operator-facing admin surface; what your `curl` /
  scripts call.
- `17129` (admin-internal gRPC) — peer-to-peer cluster RPCs only; carries
  the Suspend / Resume / Forward fan-out between OAP nodes during a
  STRUCTURAL apply or a non-main forward. NEVER expose to operators or
  to the agent network.

Both are configured under the `admin-server.default:` block in
`application.yml` (`host`/`port` for HTTP, `gRPCHost`/`gRPCPort` for the
admin gRPC bus). The `receiver-runtime-rule` block keeps the
runtime-rule-specific knobs only — periodic-scan interval and self-heal
threshold.

> **Why a dedicated `17129` port for cluster RPCs.** Privileged admin
> RPCs (Suspend / Resume / Forward) used to ride the public agent /
> cluster gRPC port (`core.gRPCPort`, default `11800`). That port is
> reachable from the agent network for legitimate telemetry submission,
> which means an attacker on the agent network could in principle invoke
> Suspend/Forward. `admin-server.gRPCPort` is bound to a private
> peer-to-peer interface only, so the admin RPCs no longer share a blast
> radius with agent telemetry.
...
| GET    | `/runtime/rule/status?catalog=&name=[&applyId=][&contentHash=]` | Reports the progress of a structural `/addOrUpdate` apply, served by the cluster main. Query by the `applyId` returned in the `structural_applied` response, or by `catalog`+`name` (+ optional `contentHash`) once the handle is gone (page refresh / main restart). Always `200` with a JSON status `{found, phase, applyId, contentHash, failureReason?, fenceLaggards?, startedAtMs, updatedAtMs, servedBy}`; `phase` is one of `PENDING`, `DDL`, `FENCING`, `ROLLING_OUT`, `APPLIED`, `DEGRADED`, `FAILED`, `UNKNOWN`. The `/addOrUpdate` response returns at `FENCING` (rule row durably persisted) with the `applyId`; dispatch for that rule stays paused while the background fence confirms cluster-wide schema propagation — because an un-propagated write is silently dropped — and only then does it advance to `ROLLING_OUT` (finalize commit, resume + notify peers) and `APPLIED`. On a genuine laggard it advances to `DEGRADED` with `fenceLaggards` (the data-node ids that did not confirm within `deferredFenceTimeoutSeconds`) and resumes dispatch anyway. When the live status is gone (or the main is unreachable) it degrades to the durable rule row: a matching `ACTIVE` row reports `phase=APPLIED` with `derivedFrom=durable-dao`. |
