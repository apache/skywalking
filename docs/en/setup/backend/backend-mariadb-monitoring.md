# MariaDB monitoring
## MariaDB server performance from `prometheus/mysqld_exporter`
...
### Data flow
1. mysqld_exporter collect metrics data from MariaDB.
...
2. Set up [OpenTelemetry Collector ](https://opentelemetry.io/docs/collector/getting-started/#docker). For details on Prometheus Receiver in OpenTelemetry Collector, refer to [here](../../../../test/e2e-v2/cases/mariadb/prometheus-mariadb-exporter/otel-collector-config.yaml).
3. Config SkyWalking [OpenTelemetry receiver](opentelemetry-receiver.md).

### MariaDB Monitoring
MariaDB monitoring provides monitoring of the status and resources of the MariaDB server. MariaDB cluster is cataloged as a `Layer: MYSQL` `Service` in OAP.
Each MariaDB server is cataloged as an `Instance` in OAP.
#### Supported Metrics 
| Monitoring Panel | Unit | Metric Name | Description | Data Source |
|-----|------|-----|-----|-----|
| MariaDB Uptime |   day   | meter_mysql_uptime | The MariaDB startup time | mysqld_exporter|
...
| Connects |    | meter_mysql_connects_available<br />meter_mysql_connects_aborted | The number of available connections(connects_available)<br/>The number of MariaDB instance connection rejections(connects_aborted)| mysqld_exporter|
...
### Data flow
1. fluentbit agent collects slow sql logs from MariaDB.
...
2. Config [fluentbit](../../../../test/e2e-v2/cases/mariadb/mariadb-slowsql/fluent-bit.conf)
3. Config MariaDB to enable slow log.[example](../../../../test/e2e-v2/cases/mariadb/mariadb-slowsql/my.cnf).

### Slow SQL Monitoring
Slow SQL monitoring provides monitoring of the slow SQL statements of the MariaDB server. MariaDB server is cataloged as a `Layer: MYSQL` `Service` in OAP.
