...
| configuration-discovery | default | disableMessageDigest | If the current configuration is true, agent receive the latest configuration even without change. In default, OAP uses SHA512 message digest mechanism to detect changes of configuration. | SW_DISABLE_MESSAGE_DIGEST | false
