...
You could specify `service` attribute by `labels` keyword if using Prometheus receiver , as following :
     ...
     scrape_configs:
       - job_name: 'apisix-monitoring' 
         static_configs:
           - targets: ['apisix:9091']
             labels:
              service: exmple_service_name  # Specify Skywalking Service name              
```
