...
`plugin.http.excude_http_headers`| When `COLLECT_HTTP_PARAMS` is enabled, control what header data should not collect, this is for security purpose, values must be lowed case,multi values split by ,  | `cookie` |
