...
* Fix: the v2 MAL compiler now resolves custom layers referenced via `Layer.nameOf('NAME')` in an expression. Custom layers declared through a `layerDefinitions:` block (or `layer-extensions.yml` / the `LayerExtension` SPI) have no generated `Layer.*` static field, so the documented `expSuffix: service(['svc'], Layer.nameOf('IOT_FLEET'))` form is the only way to name them. The v1 Groovy compiler evaluated this natively, but the v2 grammar's `enumRef` only modeled the static-field form (`Layer.GENERAL`); `Layer.nameOf('X')` was mis-parsed as a metric named `Layer` with a `.nameOf("X")` method chain and failed code generation with `nameOf(java.lang.String) not found in SampleFamily`. The parser now reinterprets the `EnumType.method('arg')` shape — keyed on the known enum-type set, so real `metric.method('x')` calls are unaffected — and emits a static call on the enum FQCN.
