...
* Document the BanyanDB trace retention pipeline (`pipeline` settings and the first-party sampler config keys) in the BanyanDB storage guide, and add an "Other trace sampling mechanisms" section to the trace-sampling guide covering the previously undocumented Zipkin receiver `sampleRate` / `maxSpansPerSecond` and how the ingest-side and merge-side samplers combine — they are independent gates, so enabling both multiplies the drop rate.
