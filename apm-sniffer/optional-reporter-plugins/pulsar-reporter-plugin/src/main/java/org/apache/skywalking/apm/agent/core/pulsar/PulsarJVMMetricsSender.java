...
/**
 * A report to send JVM Metrics data to Pulsar.
 */
@OverrideImplementor(JVMMetricsSender.class)
public class PulsarJVMMetricsSender extends JVMMetricsSender {
        ...
        if (!queue.isEmpty()) {
            ...
            if (running) {
                ...
                if (LOGGER.isDebugEnable()) {
                    LOGGER.debug(
                            "JVM metrics reporting, topic: {}, key: {}, length: {}", producer.getTopic(),
    ...
    @Override
    public void boot() {
        producer = ServiceManager.INSTANCE.findService(PulsarProducerManager.class).getProducer(Pulsar.TOPIC_METRICS);
        running = true;
