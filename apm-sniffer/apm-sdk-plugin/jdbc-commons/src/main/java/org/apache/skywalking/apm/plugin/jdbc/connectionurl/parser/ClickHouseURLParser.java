...
public class ClickHouseURLParser extends AbstractURLParser {
    ...
    private static final String DB_TYPE = "ClickHouse";
    ...
    public ConnectionInfo parse() {
        ...
        if (hostSegment.length > 1) {
            ...
            return new ConnectionInfo(ComponentsDefine.CLICKHOUSE_JDBC, DB_TYPE, sb.toString(), fetchDatabaseNameFromURL());
        } else {
            String[] hostAndPort = hostSegment[0].split(":");
            if (hostAndPort.length != 1) {
                return new ConnectionInfo(ComponentsDefine.CLICKHOUSE_JDBC, DB_TYPE, hostAndPort[0], Integer.valueOf(hostAndPort[1]), fetchDatabaseNameFromURL(location.endIndex()));
            } else {
                return new ConnectionInfo(ComponentsDefine.CLICKHOUSE_JDBC, DB_TYPE, hostAndPort[0], DEFAULT_PORT, fetchDatabaseNameFromURL(location.endIndex()));
            }
        }
    }
