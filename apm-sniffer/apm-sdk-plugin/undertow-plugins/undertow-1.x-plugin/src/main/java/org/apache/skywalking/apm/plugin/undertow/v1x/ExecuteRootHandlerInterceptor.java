...
package org.apache.skywalking.apm.plugin.undertow.v1x;
...
public class ExecuteRootHandlerInterceptor implements StaticMethodsAroundInterceptor {
    ...
    public void beforeMethod(Class clazz, Method method, Object[] allArguments, Class<?>[] parameterTypes, MethodInterceptResult result) {
        ...
        String spanName = exchange.getRequestPath();
        if (!StringUtil.isEmpty(exchange.getQueryString())) {
            spanName = String.format("%s?%s", exchange.getRequestPath(), exchange.getQueryString());
        }
        AbstractSpan span = ContextManager.createEntrySpan(spanName, contextCarrier);
