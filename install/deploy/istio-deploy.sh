...
deploy_istio() {
  ...
  helm install istio istio -n ${NAMESPACE} --set prometheus.enabled=false --set pilot.autoscaleEnabled=false \
          ...
          --set gateways.istio-ingressgateway.resources.requests.cpu=50m,gateways.istio-ingressgateway.resources.requests.memory=128m \
          --set gateways.istio-ingressgateway.resources.limits.cpu=50m,gateways.istio-ingressgateway.resources.limits.memory=128m \
          --set pilot.resources.requests.cpu=50m,pilot.resources.requests.memory=128m \
          --set pilot.resources.limits.cpu=50m,pilot.resources.limits.memory=128m \
          --set mixer.telemetry.resources.requests.cpu=50m,mixer.telemetry.resources.requests.memory=128m \
          --set mixer.telemetry.resources.limits.cpu=50m,mixer.telemetry.resources.limits.memory=128m \
          --set pilot.resources.requests.cpu=50m,pilot.resources.requests.memory=128m \
          --set pilot.resources.limits.cpu=50m,pilot.resources.limits.memory=128m \
          --set global.defaultResources.requests.cpu=50m,global.defaultResources.requests.memory=128m \
          --set global.defaultResources.limits.cpu=50m,global.defaultResources.limits.memory=128m \
