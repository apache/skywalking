 ...
 */
public class ApplicationReferenceH2MetricPersistenceDAO extends H2DAO implements IApplicationReferenceMetricPersistenceDAO<H2SqlEntity, H2SqlEntity, ApplicationReferenceMetric> {

    private final Logger logger = LoggerFactory.getLogger(ApplicationReferenceH2MetricPersistenceDAO.class);
    private static final String GET_SQL = "select * from {0} where {1} = ?";

    public ApplicationReferenceH2MetricPersistenceDAO(H2Client client) {
        super(client);
    }
    ...
    @Override public ApplicationReferenceMetric get(String id) {
        ...
        try (ResultSet rs = client.executeQuery(sql, params)) {
            if (rs.next()) {
                ...
                applicationReferenceMetric.setFrontApplicationId(rs.getInt(NodeReferenceTable.COLUMN_FRONT_APPLICATION_ID));
                applicationReferenceMetric.setBehindApplicationId(rs.getInt(NodeReferenceTable.COLUMN_BEHIND_APPLICATION_ID));
                applicationReferenceMetric.setS1Lte(rs.getInt(NodeReferenceTable.COLUMN_S1_LTE));
                applicationReferenceMetric.setS3Lte(rs.getInt(NodeReferenceTable.COLUMN_S3_LTE));
                applicationReferenceMetric.setS5Lte(rs.getInt(NodeReferenceTable.COLUMN_S5_LTE));
                applicationReferenceMetric.setS5Gt(rs.getInt(NodeReferenceTable.COLUMN_S5_GT));
                applicationReferenceMetric.setSummary(rs.getInt(NodeReferenceTable.COLUMN_SUMMARY));
                applicationReferenceMetric.setError(rs.getInt(NodeReferenceTable.COLUMN_ERROR));
                applicationReferenceMetric.setTimeBucket(rs.getLong(NodeReferenceTable.COLUMN_TIME_BUCKET));
