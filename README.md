...
* 通过配置文件支持的监控插件
1 web-plugin
2 dubbo-plugin(support dubbo/dubbox)
3 spring-plugin
4 jdbc-plugin
* 通过动态字节码机制支持的监控插件
1 mysql-plugin（和jdbc-plugin实现模式不同，对于mysql等效）
2 httpClient-4.x-plugin，同时支持4.2和4.3。原有的httpclient-4.2.x-plugin，httpclient-4.3.x-plugin由比较严重的代码侵入，将在适当以后予以移除。
