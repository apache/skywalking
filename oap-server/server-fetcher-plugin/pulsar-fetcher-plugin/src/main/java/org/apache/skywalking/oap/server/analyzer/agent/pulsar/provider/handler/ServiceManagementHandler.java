 ...
 */
@Slf4j
public class ServiceManagementHandler implements PulsarHandler {
    ...
    public ServiceManagementHandler(ModuleManager moduleManager, PulsarFetcherConfig config) {
    ...
    public void handle(final Message msg) {
        try {
            if (msg.getKey().startsWith("register-")) {
                serviceReportProperties(InstanceProperties.parseFrom(msg.getData()));
            } else {
                keepAlive(InstancePingPkg.parseFrom(msg.getData()));
            }
        } catch (Exception e) {
