...
public class JDBCAlarmQueryDAO implements IAlarmQueryDAO {
    ...
    @Override
    @SneakyThrows
    public Alarms queryAlarms(final AlarmQueryCondition condition, final int limit, final int from) {
        ...
        final List<Tag> tags = condition.getTags();
        if (tags != null && !searchableTagKeys.containsAll(tags.stream().map(Tag::getKey).collect(toSet()))) {
            log.warn(
                    "Searching tags that are not searchable: {}",
                    tags.stream().map(Tag::getKey).filter(not(searchableTagKeys::contains)).collect(toSet()));
        ...
        final var alarmMsgs = new ArrayList<AlarmMessage>();
        for (final var table : tables) {
            final var sqlAndParameters = buildSQLForQueryAlarms(condition, entityConstraints, limit, from, table);
            jdbcClient.executeQuery(sqlAndParameters.sql(), resultSet -> {
                while (resultSet.next()) {
                    AlarmRecord.Builder builder = new AlarmRecord.Builder();
                    Convert2Entity convert2Entity = JDBCEntityConverters.toEntity(resultSet);
    ...
    /**
     * Build the SQL for {@link #queryAlarms}. Mirrors {@link #buildSQL} structure
     * but uses the comprehensive condition object — adds entity-IDs / layers /
     * ruleNames filters on top of the legacy fields.
     */
    protected SQLAndParameters buildSQLForQueryAlarms(final AlarmQueryCondition condition,
                                                      final List<EntityIdConstraint> entityConstraints,
        ...
        if (CollectionUtils.isNotEmpty(condition.getRuleNames())) {
            ...
            parameters.addAll(condition.getRuleNames());
        }
        if (CollectionUtils.isNotEmpty(entityConstraints)) {
            // Per-entity constraints OR together; within each constraint the
            // id0 / id1 predicates AND together. For non-relation entities the
            // resolver emits two constraints — (id0=X) and (id1=X) — so they
            // OR back to "primary OR relation-dest". For relation entities a
            // single constraint with both columns set ensures an exact match.
            final var clauses = new ArrayList<String>(entityConstraints.size());
            for (final EntityIdConstraint c : entityConstraints) {
                final var parts = new ArrayList<String>(2);
                if (c.getId0() != null) {
                    parts.add(AlarmRecord.ID0 + " = ?");
                    parameters.add(c.getId0());
                }
                if (c.getId1() != null) {
                    parts.add(AlarmRecord.ID1 + " = ?");
                    parameters.add(c.getId1());
                }
                if (!parts.isEmpty()) {
