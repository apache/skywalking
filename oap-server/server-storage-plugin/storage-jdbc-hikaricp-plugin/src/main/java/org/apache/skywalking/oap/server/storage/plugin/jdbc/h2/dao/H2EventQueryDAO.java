...
public class H2EventQueryDAO implements IEventQueryDAO {
    ...
    public Events queryEvents(List<EventQueryCondition> conditionList) throws Exception {
        ...
        for (EventQueryCondition condition : conditionList) {
            ...
            whereClauses.add(conditions.stream().collect(Collectors.joining(" and ", "", "")));
            conditions.clear();
        }
        if (CollectionUtils.isNotEmpty(whereClauses)) {
            whereClauseBuilder.append(whereClauses.stream().collect(Collectors.joining(") or ( ", "", "")));
        }
        String whereClause = whereClauseBuilder.append(")").toString();
