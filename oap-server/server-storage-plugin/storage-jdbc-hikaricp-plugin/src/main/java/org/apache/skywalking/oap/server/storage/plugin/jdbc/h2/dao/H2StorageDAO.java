...
public class H2StorageDAO implements StorageDAO {
    ...
    public IMetricsDAO newMetricsDao(StorageBuilder storageBuilder) {
        return new H2MetricsDAO(config, h2Client, (StorageHashMapBuilder<Metrics>) storageBuilder);
    }

    @Override
    public IRecordDAO newRecordDao(StorageBuilder storageBuilder) {
        return new H2RecordDAO(config, manager, h2Client, (StorageHashMapBuilder<Record>) storageBuilder);
    }

    @Override
    public INoneStreamDAO newNoneStreamDao(StorageBuilder storageBuilder) {
        return new H2NoneStreamDAO(config, h2Client, (StorageHashMapBuilder<NoneStream>) storageBuilder);
    }

    @Override
    public IManagementDAO newManagementDao(StorageBuilder storageBuilder) {
        return new H2ManagementDAO(config, h2Client, (StorageHashMapBuilder<ManagementData>) storageBuilder);
    }
}
