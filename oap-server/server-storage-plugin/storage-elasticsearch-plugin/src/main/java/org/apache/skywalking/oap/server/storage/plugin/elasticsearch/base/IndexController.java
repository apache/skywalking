...
public enum IndexController {
    ...
    public static class LogicIndicesRegister {
        ...
        public static String getPhysicalTableName(String logicName) {
            final String catalogEntry = LOGIC_INDICES_CATALOG.get(logicName);
            if (catalogEntry != null) {
                return catalogEntry;
            }
            // Provide-if-absent: the catalog wins (above). A foreign metric (inspect overlay active on
            // this thread, no local registry entry) lives in the single merged metrics index instead of
            // a per-metric index that does not exist; isMergedTable() derives true from this. The same
            // overlay gates the foreign handling in MetricsQueryEsDAO. logicSharding is rejected upstream
            // in the DAO, where the request context is understood.
            final ForeignMetricMeta foreign = InspectQueryContext.get(logicName);
            return foreign != null ? METRICS_LOGIC_TABLE_NAME : logicName;
        }
