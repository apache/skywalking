...
package org.apache.skywalking.oap.log.analyzer.v2.dsl.debug;
 ...
 * binding without orphaning it. Two things hold that identity: both routes
 * derive the field through {@code LALConfigs.stampSource} (pass
 * {@code sourcePath} here instead and every hot update strands the static
 * binding it was meant to replace), and {@code RuleKey} canonicalises the
 * file name, so the extension this route carries and the bare name the
 * runtime-rule engine publishes resolve to one entry.
 */
