...
public class ConversationFileBuilder implements LALOutputBuilder {
    ...
    /** The size limit before the module's configuration applies one, and after a test that lowered it. */
    public static final long DEFAULT_MAX_FILE_BYTES = 15L * 1024 * 1024;
    ...
    private static volatile CounterMetrics REJECTED_SIZE;
    private static volatile long MAX_FILE_BYTES = DEFAULT_MAX_FILE_BYTES;
     ...
     * @param size          files rejected because the body is larger than the limit
     */
    ...
    /**
     * The largest body stored. One file over the storage's message limit fails the bulk write it travels in
     * and every record behind it in that write, so the limit is applied here, where one file is one record.
     *
     * @param bytes the limit, in bytes
     */
    public static void setMaxFileBytes(final long bytes) {
    ...
    public void complete(final SourceReceiver sourceReceiver) {
        ...
        if (body.length > MAX_FILE_BYTES) {
            reject(REJECTED_SIZE, "the body is " + body.length + " bytes, over the limit of " + MAX_FILE_BYTES);
            return;
        }
