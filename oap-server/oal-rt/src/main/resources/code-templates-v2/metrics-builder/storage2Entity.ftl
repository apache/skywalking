...
<#list persistentFields as field>
    ...
    <#assign fieldSetter = "set" + field.fieldName?cap_first>
    <#if fieldTypeName == "long" || fieldTypeName == "int" || fieldTypeName == "double" || fieldTypeName == "float">
        metrics.${fieldSetter}(((Number)converter.get("${field.columnName}")).${fieldTypeName}Value());
    <#elseif fieldTypeName == "java.lang.String">
        metrics.${fieldSetter}((String)converter.get("${field.columnName}"));
    <#else>
        metrics.${fieldSetter}(new ${fieldTypeName}((String)converter.get("${field.columnName}")));
    </#if>
</#list>
return metrics;
}
