...
public class DispatcherManager implements DispatcherDetectorListener {
    ...
    /**
     * Resolve the {@link GateHolder} for an OAL metric. OAL gates are
     * per-metric — every {@code do<Metric>} on a generated dispatcher has its
     * own holder. A session targets one metric and captures only that
     * metric's pipeline; the other metrics on the same dispatcher stay
     * silent.
     *
     * <p>Dispatchers generated by {@code OALClassGeneratorV2} implement
     * {@link DebugHolderProvider#debugHolder(String)}. We walk every
     * dispatcher and ask each in turn — the dispatcher whose
     * {@code debugHolder(metricName)} returns non-null owns the rule.
     *
     * @return the live holder for the metric, or {@code null} if no
     *         dispatcher carries a rule with that name.
     */
    public GateHolder debugHolderForOalMetric(final String metricName) {
        if (metricName == null || metricName.isEmpty()) {
            return null;
        }
        for (final List<SourceDispatcher<ISource>> bucket : dispatcherMap.values()) {
            for (final SourceDispatcher<ISource> dispatcher : bucket) {
                if (!(dispatcher instanceof DebugHolderProvider)) {
                    continue;
                }
                final GateHolder holder =
                    ((DebugHolderProvider) dispatcher).debugHolder(metricName);
                if (holder != null) {
                    return holder;
                }
            }
    ...
    /**
     * Snapshot every {@link SourceDispatcher} the manager currently routes
     * to. Read-only, defensive copy — used by debug-API integration paths
     * that need to walk the dispatcher set without holding the manager's
     * write lock.
     */
    public List<SourceDispatcher<ISource>> snapshotDispatchers() {
        final List<SourceDispatcher<ISource>> all = new ArrayList<>();
        for (final List<SourceDispatcher<ISource>> bucket : dispatcherMap.values()) {
            all.addAll(bucket);
        }
