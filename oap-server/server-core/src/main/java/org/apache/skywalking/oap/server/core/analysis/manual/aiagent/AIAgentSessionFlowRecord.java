...
public class AIAgentSessionFlowRecord extends Record {
    ...
    public static final int BODY_MAX_LENGTH = 16 * 1024 * 1024;
    ...
    /** Sortable, so the head of a chain is one read of the highest round rather than a page of rows. */
    ...
    /**
     * The file as landed. The length is the most raw bytes a body may hold, BanyanDB's gRPC message limit; the
     * ai-agent-conversation module admits less, and a JDBC storage sizes the column by it, as Base64.
     */
    @Column(name = BODY, storageOnly = true, length = BODY_MAX_LENGTH)
    private byte[] body;
