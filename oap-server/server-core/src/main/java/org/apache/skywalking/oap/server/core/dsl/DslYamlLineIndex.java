...
/**
 * Resolves the 1-based YAML line of each rule in a DSL rule file.
 *
 * <p>SnakeYAML's bean binding ({@code Yaml.loadAs}) discards positional {@code Mark} data, so the
 * rule objects the loaders hand to {@code MetricConvert} carry no idea where they came from. This
 ...
 */
@Slf4j
public final class DslYamlLineIndex {
    ...
    private static final DslYamlLineIndex EMPTY =
        new DslYamlLineIndex(0, 0, 0, Collections.emptyList());
    ...
    private DslYamlLineIndex(final int filterLine, final int expPrefixLine,
     ...
     */
    public static DslYamlLineIndex index(final String yamlContent, final String rulesKey) {
        ...
        try (StringReader reader = new StringReader(yamlContent)) {
            ...
            return new DslYamlLineIndex(filter, prefix, suffix, ruleLines);
    ...
    /**
     * 1-based line of every KEY under a mapping section, e.g. {@code auto-matching-rules}.
     *
     * <p>Sequence-shaped rule files are matched positionally by {@link #rule(int)}; a mapping has
     * no positions, so its entries are keyed by name. Hierarchy is the only DSL shaped this way.
     *
     * @param yamlContent the exact text the config was bound from
     * @param sectionKey  the mapping's key, e.g. {@code auto-matching-rules}
     * @return key to 1-based line; empty when the section is absent or unparseable
     */
    public static Map<String, Integer> keyLines(final String yamlContent,
                                                          final String sectionKey) {
        final Map<String, Integer> out = new LinkedHashMap<>();
        if (yamlContent == null || sectionKey == null) {
            return out;
        }
        try {
            final Node root = new Yaml(new LoaderOptions()).compose(new StringReader(yamlContent));
            if (!(root instanceof MappingNode)) {
                return out;
            }
            for (final NodeTuple tuple : ((MappingNode) root).getValue()) {
                if (!(tuple.getKeyNode() instanceof ScalarNode)
                    || !sectionKey.equals(((ScalarNode) tuple.getKeyNode()).getValue())) {
                    continue;
                }
                if (!(tuple.getValueNode() instanceof MappingNode)) {
                    return out;
                }
                for (final NodeTuple entry : ((MappingNode) tuple.getValueNode()).getValue()) {
                    if (entry.getKeyNode() instanceof ScalarNode) {
                        out.put(((ScalarNode) entry.getKeyNode()).getValue(),
                            entry.getKeyNode().getStartMark().getLine() + 1);
                    }
                }
                return out;
            }
        } catch (final Exception e) {
            log.debug("Could not index keys under {}: {}", sectionKey, e.getMessage());
        }
        return out;
    }
}
