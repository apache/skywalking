...
public class CountFunctionTest {
    ...
    @BeforeEach
    public void before() {
        function = new CountFunctionInst();
    ...
    public void testAccept() {
        ...
        assertThat(function.getValue()).isEqualTo(1);

        function.accept(MeterEntity.newService("service-test", Layer.GENERAL), SMALL_VALUE);
        assertThat(function.getValue()).isEqualTo(2);
    }
    ...
    public void testToHour() {
        ...
        final CountFunction hourFunction = (CountFunction) function.toHour();
        hourFunction.calculate();

        assertThat(hourFunction.getValue()).isEqualTo(2);
    }
    ...
    public void testToDay() {
        ...
        final CountFunction dayFunction = (CountFunction) function.toDay();
        dayFunction.calculate();
        assertThat(dayFunction.getValue()).isEqualTo(2);
    }

    @Test
    public void testSerialize() {
        function.accept(MeterEntity.newService("service-test", Layer.GENERAL), LARGE_VALUE);
        CountFunction function2 = new CountFunctionInst();
    ...
    public void testBuilder() throws IllegalAccessException, InstantiationException {
        function.accept(MeterEntity.newService("service-test", Layer.GENERAL), LARGE_VALUE);
        function.calculate();
        StorageBuilder<CountFunction> storageBuilder = function.builder().newInstance();

        final HashMapConverter.ToStorage toStorage = new HashMapConverter.ToStorage();
        storageBuilder.entity2Storage(function, toStorage);
        final Map<String, Object> map = toStorage.obtain();
        map.put(CountFunction.VALUE, map.get(CountFunction.VALUE));

        CountFunction function2 = storageBuilder.storage2Entity(new HashMapConverter.ToEntity(map));

        assertThat(function2.getValue()).isEqualTo(function.getValue());
    }

    private static class CountFunctionInst extends CountFunction {
        @Override
        public AcceptableValue<Long> createNew() {
            return new CountFunctionInst();
        }
    }
}
