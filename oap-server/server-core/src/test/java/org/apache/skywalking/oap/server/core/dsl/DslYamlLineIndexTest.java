...
class DslYamlLineIndexTest {
    ...
    void resolvesFileLevelAnchors() {
        final DslYamlLineIndex index = DslYamlLineIndex.index(YAML);
    ...
    void honoursAnAlternateRulesKey() {
        ...
        assertEquals(3, DslYamlLineIndex.index(zabbix, "metrics").rule(0).getEntryLine());
        assertEquals(4, DslYamlLineIndex.index(zabbix, "metrics").rule(0).getExpLine());
        // The default key finds nothing in that document — zeros, not an exception.
        assertEquals(0, DslYamlLineIndex.index(zabbix).rule(0).getEntryLine());
    }

    @Test
    void malformedOrEmptyInputDegradesToZeros() {
        // A missing line number must never stop a rule from compiling.
        assertEquals(0, DslYamlLineIndex.index(null).getFilterLine());
        assertEquals(0, DslYamlLineIndex.index("").getFilterLine());
        assertEquals(0, DslYamlLineIndex.index("- not: a mapping root").getFilterLine());
        assertEquals(0, DslYamlLineIndex.index("key: [unclosed").getFilterLine());
    }
