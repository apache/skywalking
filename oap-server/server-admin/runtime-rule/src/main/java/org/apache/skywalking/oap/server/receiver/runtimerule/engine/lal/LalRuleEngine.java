...
public final class LalRuleEngine implements RuleEngine<LalApplyContext> {
    ...
    private static LalFileApplier.Applied appliedFor(final Map<String, AppliedRuleScript> rules,
        ...
        final EngineApplied a = script.getApplied();
    ...
    /**
     * Resolve the dsl-debugging module's {@link LALHolderRegistry} so commit / unregister can
     * publish the live {@code GateHolder} per LAL rule for the DSL debug API to bind into.
     * Returns {@code null} when dsl-debugging is not enabled — runtime-rule treats the
     * registry as optional, mirroring the MAL engine's resolveMalHolderRegistry approach.
     */
    private LALHolderRegistry resolveLalHolderRegistry() {
        try {
            return moduleManager.find(DSLDebuggingModule.NAME).provider()
                .getService(LALHolderRegistry.class);
        } catch (final Throwable t) {
            return null;
        }
    }
    ...
    /**
     * Publish every LAL rule's debug holder on the dsl-debugging registry. One {@link RuleKey}
     * per (catalog, file-name, rule-name) — same identity the session install path uses.
     * Idempotent on re-apply (registry uses put-replace).
     *
     * <p>The {@code Applied}'s {@code RegisteredRule} list pairs each rule's layer / name with
     * the {@code CompiledLAL.dsl} kept inside the LogFilterListener.Factory; the LAL engine
     * doesn't hold those compiled objects directly. To keep the engine boundary clean we
     * resolve each binding through the factory, asking it for the live DSL keyed on
     * {@code (layer, ruleName)} immediately after a successful apply.
     */
    private void publishDebugBindings(final String fileName,
                                      final LalFileApplier.Applied applied) {
        if (applied == null || applied.getRegistered().isEmpty()) {
            return;
        }
        final LALHolderRegistry registry = resolveLalHolderRegistry();
        if (registry == null) {
            return;
        }
        final LogFilterListener.Factory factory = resolveFactory();
        if (factory == null) {
            return;
        }
        for (final LalFileApplier.RegisteredRule r : applied.getRegistered()) {
            final DSL dsl = factory.find(r.getLayer(), r.getRuleName());
            if (dsl == null || dsl.getExpression() == null) {
                continue;
            }
            registry.register(
                new RuleKey(Catalog.LAL, fileName, r.getRuleName()),
                dsl.getExpression().debugHolder());
        }
    }

    /**
     * Reverse of {@link #publishDebugBindings}. Iterates the prior {@code Applied}'s rule
     * keys and drops each binding so a hot-update doesn't strand a stale entry.
     */
    private void unpublishDebugBindings(final String fileName,
                                        final LalFileApplier.Applied applied) {
        if (applied == null || applied.getRegistered().isEmpty()) {
            return;
        }
        ...
        for (final LalFileApplier.RegisteredRule r : applied.getRegistered()) {
            registry.unregister(new RuleKey(Catalog.LAL, fileName, r.getRuleName()));
        }
    }

    /** Resolve the live LAL listener factory; same lazy try/catch pattern as {@link #resolveApplier}. */
    private LogFilterListener.Factory resolveFactory() {
        try {
            return moduleManager.find(LogAnalyzerModule.NAME).provider()
    ...
    public void commit(final CompiledDSL compiled, final LalApplyContext ctx) {
        ...
        // Drop prior debug bindings so the registry doesn't strand stale GateHolder refs
        // for rules this commit no longer owns; pre-update sessions stay safe via
        // DebugSession.boundHolder.
        if (c.getOldApplied() != null) {
            unpublishDebugBindings(c.getName(), c.getOldApplied());
        }
        publishDebugBindings(c.getName(), c.getNewApplied());
    ...
    public void unregister(final String catalog, final String name, final LalApplyContext ctx) {
        ...
        if (priorLal != null) {
            // Drop debug bindings BEFORE we wipe the Applied — the registry holds GateHolder
            // refs by reference, and pre-update sessions stay safe via DebugSession.boundHolder.
            unpublishDebugBindings(name, priorLal);
    ...
    public boolean installBundled(final String catalog, final String name,
        ...
        try {
            ...
            publishDebugBindings(name, fresh);
