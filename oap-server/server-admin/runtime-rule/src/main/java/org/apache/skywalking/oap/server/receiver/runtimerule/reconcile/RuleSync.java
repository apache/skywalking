...
public final class RuleSync {
    ...
    private void applyOneFromDb(final RuntimeRuleManagementDAO.RuntimeRuleFile ruleFile,
        ...
        // Lock-free fast path: skip the lock acquisition altogether when the
        // pre-lock snapshot already shows no change. The post-lock re-read below
        // is the load-bearing one — this is just a tick-perf optimization to
        // avoid contending on perFile when most rules are quiescent.
        final AppliedRuleScript snapshotScript = rules.get(key);
        final DSLRuntimeState snapshotPrev = snapshotScript == null ? null : snapshotScript.getState();
        final boolean dbActive = !"INACTIVE".equals(ruleFile.getStatus());
        final boolean localEffectivelyActive = snapshotPrev != null
            && snapshotPrev.getLocalState() != DSLRuntimeState.LocalState.NOT_LOADED;
        if (snapshotPrev != null
                && dbActive == localEffectivelyActive
                && Objects.equals(snapshotPrev.getContentHash(), newHash)) {
            return;
        }
        ...
        try (HistogramMetrics.Timer ignored = lockMetrics.startSyncTimerHoldTimer()) {
            // Re-read prev UNDER the lock — a racing REST write or earlier
            // sync tick may have stamped a new state between the snapshot read
            // above and the lock acquisition. Re-checking the no-change
            // short-circuit here drops races where the REST handler already
            // applied this exact (hash, status) tuple while we were waiting.
            final AppliedRuleScript prevScript = rules.get(key);
            final DSLRuntimeState prev = prevScript == null ? null : prevScript.getState();
            final boolean lockedDbActive = !"INACTIVE".equals(ruleFile.getStatus());
            final boolean lockedLocalActive = prev != null
                && prev.getLocalState() != DSLRuntimeState.LocalState.NOT_LOADED;
            if (prev != null
                    && lockedDbActive == lockedLocalActive
