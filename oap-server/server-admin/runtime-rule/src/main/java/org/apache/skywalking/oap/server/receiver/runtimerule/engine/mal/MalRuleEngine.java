...
public final class MalRuleEngine implements RuleEngine<MalApplyContext> {
    ...
    /**
     * Resolve the dsl-debugging module's {@link MALHolderRegistry} so commit / unregister can
     * publish the live {@code MalExpression} per metric for the DSL debug API to bind into.
     * Returns {@code null} when the dsl-debugging module is not enabled — runtime-rule treats
     * the registry as optional, which is why dsl-debugging is NOT in {@link #requiredModules
     * requiredModules()} of the runtime-rule provider.
     */
    private MALHolderRegistry resolveMalHolderRegistry() {
        try {
            return moduleManager.find(DSLDebuggingModule.NAME).provider()
                .getService(MALHolderRegistry.class);
        } catch (final Throwable t) {
            return null;
        }
    }
    ...
    /**
     * Publish every analyzer's MalExpression on the dsl-debugging registry so a debug session
     * can resolve the rule's {@code GateHolder}. One {@link RuleKey} per metric — same identity
     * the session install path uses. Idempotent on re-apply (registry uses put-replace).
     */
    private void publishDebugBindings(final String catalog, final String name,
                                      final MetricConvert convert) {
        if (convert == null) {
            return;
        }
        final MALHolderRegistry registry = resolveMalHolderRegistry();
        if (registry == null) {
            return;
        }
        final Catalog catalogEnum;
        try {
            catalogEnum = Catalog.of(catalog);
        ...
        for (final Analyzer analyzer : convert.getAnalyzers()) {
            ...
            registry.register(
                new RuleKey(catalogEnum, name, analyzer.getMetricName()),
                analyzer.getExpression().debugHolder());
        }
    }

    /**
     * Reverse of {@link #publishDebugBindings}. Called from unregister / commit-overwrite paths
     * so a hot-update doesn't strand a stale binding the session registry would still resolve.
     * Pre-update sessions remain bound to their original {@code GateHolder} via
     * {@code DebugSession.boundHolder} — unregister here only affects fresh installs.
     */
    private void unpublishDebugBindings(final String catalog, final String name,
        ...
        for (final String metric : metricNames) {
            registry.unregister(new RuleKey(catalogEnum, name, metric));
        }
    }
    ...
    public void commit(final CompiledDSL compiled, final MalApplyContext ctx) {
        ...
        // Drop the prior bindings so the registry doesn't strand a stale MalExpression for
        // metrics this commit no longer claims (STRUCTURAL/NEW only — FILTER_ONLY's metric
        // set is identical, and unpublishing then republishing is a wasted round-trip).
        if (c.getClassification() != Classification.FILTER_ONLY && c.getOldApplied() != null) {
            unpublishDebugBindings(
                c.getCatalog(), c.getName(),
                c.getOldApplied().getRegisteredMetricNames());
        }
        publishDebugBindings(
            c.getCatalog(), c.getName(), c.getNewApplied().getMetricConvert());
    ...
    public void unregister(final String catalog, final String name, final MalApplyContext ctx) {
        ...
        if (priorMal != null) {
            // Drop debug bindings BEFORE we wipe the Applied — the registry holds onto the
            // MalExpression by reference, and pre-update sessions stay safe via boundHolder.
            unpublishDebugBindings(catalog, name, priorMal.getRegisteredMetricNames());
            final MalFileApplier applier = resolveApplier();
            if (applier == null) {
    ...
    public boolean installBundled(final String catalog, final String name,
        ...
        try {
            ...
            publishDebugBindings(catalog, name, fresh.getMetricConvert());
