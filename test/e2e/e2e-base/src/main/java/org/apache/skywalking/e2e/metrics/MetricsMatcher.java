...
public class MetricsMatcher {
    ...
    public static void verifyMetrics(SimpleQueryClient queryClient, String metricName, String id,
        ...
        boolean valid = false;
        while (!valid) {
            Metrics metrics = queryClient.metrics(new MetricsQuery().stepByMinute()
    ...
    public static void verifyPercentileMetrics(SimpleQueryClient queryClient, String metricName, String id,
        ...
        boolean valid = false;
        while (!valid) {
            List<Metrics> metricsArray = queryClient.multipleLinearMetrics(new MetricsQuery().stepByMinute()
                                                                                             .metricsName(metricName)
                                                                                             .start(minutesAgo)
                                                                                             .end(LocalDateTime.now(ZoneOffset.UTC)
                                                                                                               .plusMinutes(1))
